...
- Prompt injection through paths that do not Spotlighting-wrap vault content (the CLI thread path; the native agent's tool results wrap since 2026-07-24, "one tool surface").
...
- **PathGuard: yes on note and canvas ops.** `read_note`/`write_note`/`edit_note`/`search_vault` resolve through `resolveInVault` (PathGuard symlink + deny-list + null-byte checks). Canvas readers and writers (`read_canvas`/`pin_to_canvas`/`unpin_from_canvas`/`focus_canvas`) use the strict `CANVAS_ID_RE` regex at the `callTool` barrel as a fast first reject, then route the computed path through `resolveInVault` in `canvasFilePath` so a regex-valid `canvasId` that resolves through a symlink out of the vault returns `PATH_OUT_OF_VAULT` (QW5, 2026-06-06). The earlier "id regex only, no PathGuard backstop" gap is closed. Canvas mutations (pin/unpin plans, and `focus_canvas`'s viewport write) persist through the shared main-side applier `canvas-apply.ts` (`validateCanvasMutationOps` + `applyPlanOps` inside the per-file write queue) — the same applier `canvas.apply_plan` persists through on the MCP lane.
...
- **Audit: yes, reads included (2026-07-24, "one tool surface").** `read_note` and `edit_note`'s pre-edit read audit through the shared facade (entries labeled `read_note`/`edit_note`); `read_canvas` audits path-level through `facade.assertReadable`. Every successful write emits an append-only `AuditEntry` — note writes via the shared facade's logger (args carry `agentId: 'native-agent'` plus the native tool label), canvas writes via a per-run `AuditLogger` — all at the same `app.getPath('userData')/audit` sink as the MCP path. Note content is never logged; rejected/failed writes are not audited, and `search_vault`/`list_vault`/`list_canvases` produce no audit entries (same class as the MCP search/graph gap below). Scope note: the limiter is per-run, so it guards a single runaway turn, not cross-turn velocity (the MCP path's limiter is per-vault).
- **Provenance + echo suppression: yes (AD2, 2026-06-06; facade-level since 2026-07-24).** `write_note`/`edit_note` route their post-approval write through the shared `VaultQueryFacade.writeFile` itself — `ToolContext.facade` is the SAME instance the MCP server uses, exposed via `McpLifecycle.getFacade()` — inheriting PathGuard, audit, the `writeStampedNote` provenance stamp (`modified_by`/`modified_at`) with `DocumentManager` echo suppression (no spurious `doc:external-change` for an open note), and the read-your-writes index refresh. The two previously-drifting "write a note safely" implementations are converged at the facade; the native tool label flows into the audit entry (no double-log). `stampProvenance` serializes only the frontmatter and appends the body verbatim, so a body whose first line is `---` is never re-parsed/shattered. Creates route through the sibling `createStampedNote` (created_by/created_at stamp, exclusive `wx` create) — `VaultQueryFacade.createFile` converged onto it 2026-07-24 ("one write spine"), closing the same-class round-trip bug it previously carried.
- **Spotlighting: yes (2026-07-24, "one tool surface").** `read_note` content, `search_vault` results, and `read_canvas` snapshots wrap in the shared envelope (`src/shared/spotlighting.ts`) before reaching the model; `list_vault`/`list_canvases` return structured JSON (same exemption as `search.query`). The renderer's tool cards unwrap for display only (`unwrapSpotlighting`).
...
### `vault:emerge-ghost` (ghost synthesis, renderer-driven)

`registerGhostEmergeIpc` (`src/main/ipc/ghost-emerge.ts`) synthesizes a note from a ghost's references via the claude CLI (`claude-cli.ts` — synthesis transport only, no write access) and creates it in the vault.

- PathGuard: yes (reference reads, target folder, and target path all assert within the vault).
- HITL: yes (2026-07-24, "one write spine"): the serialized note rides `QueueHitlGate` over the global ApprovalQueue — the same 30s fail-closed gate-confirm surface as MCP writes, with the note content as the tray row's diff preview. Deny, timeout, or an unwired gate (fail-closed default) means no folder creation and no write.
- Provenance + write mechanics: `createStampedNote` (`note-write.ts`) — created_by/created_at stamp, exclusive create.
- Audit: yes — `vault.emerge_ghost` entries (allowed/denied) at the shared `userData/audit` sink.
...
| Spotlighting envelope (shared by MCP + native lanes; renderer unwraps for display) | `src/shared/spotlighting.ts` | `wrapSpotlighting`, `unwrapSpotlighting`, `SPOTLIGHT_BOUNDARY` |
| Canvas plan applier (main-side persistence, shared by MCP `canvas.apply_plan` + native canvas tools) | `src/main/services/canvas-apply.ts` | `applyCanvasPlanToFile`, `writeCanvasViewport` |
...
| Safe note-write mechanics (provenance stamp, echo suppression, atomic/exclusive write) | `src/main/utils/note-write.ts` | `writeStampedNote`, `createStampedNote` |
...
- `canvas.apply_plan` (gated, PathGuard + path-level audit via `assertReadable`; persists main-side through `canvas-apply.ts` with the mtime precondition inside the write queue, then syncs the renderer — an accepted plan lands on disk even when the canvas isn't open)

## Known gaps

Tracked here so the doc and the code stay honest with each other.

1. **Audit coverage**: `search.query`, `graph.get_neighbors`, and `graph.get_ghosts` do not log; neither do the native `search_vault`/`list_vault`/`list_canvases`. Closing this means adding read audits to the facade's search/graph methods and the native list/search tools.
